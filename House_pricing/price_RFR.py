# -*- coding: utf-8 -*-
"""
Created on Sun Mar 29 12:44:28 2020
...
print('The X_train set shape is : %s' %str(train.shape))
print('The X_test set shape is : %s' %str(test.shape))
print('The combined data set shape is : %s' %str(data.shape))


# REMOVING columns FROM TRAINING SET 
data.columns.get_loc('Condition1')
data.columns.get_loc('GarageQual')
data.columns.get_loc('Street')
data.columns.get_loc('YearBuilt')
'''
X = np.delete(X,13 , 1 )
X = np.delete(X,63 , 1 )
X = np.delete(X,5 , 1 )
...
# TRAINING SET
print(regressor.score(X_train, y_train))      # 97.76324962
# TEST SET 
print(regressor.score(X_test, y_test))        # 88.87353596
...
# -9                   -8 -7 couldn't convert             


np.unique(X[: , -5])

source_col_loc = dataset.columns.get_loc('OpenPorchSF')
dataset['Porch'] =dataset.iloc[:, source_col_loc+1:source_col_loc+4].apply(
        lambda X: ','.join(X.dropna.astype(str)), axis =1)
dataset

dataset1=   dataset.corr()
fig =plt.figure()
ax =fig.add_subplot(111)
cax =ax.matshow(dataset1 , cmap ='coolwarm' , vmin=-1 , vmax=1)
fig.colorbar(cax)
...
dataset1=   dataset.corr()
sns.heatmap(dataset1)

'''


''' CONDITION 1 , STREETS ,   YEAR BUILT , 
